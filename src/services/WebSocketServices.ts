...
function captalizeFirstLetter(string: string): string {
...
class PeerConnectionSession {
    ...
    addPeerConnection(id: string, stream: any, callback: any) {
        if (!this.peerConnections[id]) {
            ...
            this.peerConnections[id] = new window.RTCPeerConnection({
                iceServers: [{ urls: 'stun:sunt.l.goolge.com:19302' }]
            });
            ...
            this.listner[id] = (event: any) => {
                const on = '_on' + captalizeFirstLetter(this.peerConnections[id].connectionState);
                const fn = this.ons[on];
                fn && fn(event, id);
            }
            console.log('adicionou o listner', id)

            this.peerConnections[id].addEventListener('connectionstatechange', this.listner[id]);
    ...
    removePeerConnection(id: string) {
        if (this.peerConnections[id]) {
            this.peerConnections[id].removeEventListner('connectionstatechange', this.listner[id]);
            delete this.peerConnections[id];
            delete this.listner[id];
        }
    }
